...
""".format({
		output_panel = "$'/root/MainWindow/HSplitContainer/ok/OutputPanel/RichTextLabel'"
})
...
func _gui_input(event: InputEvent) -> void:
	if event.is_action_pressed("toggle_comment") && self.editable:
	...
	elif event.is_action_pressed("tab") && self.editable:
		# If no selection is active, toggle comment on the line the cursor is currently on.
		var from
		var to
		if has_selection():	
